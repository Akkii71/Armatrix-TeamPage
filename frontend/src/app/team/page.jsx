...
export default function TeamPage() {
  ...
  return (
    <div className="relative min-h-screen bg-[#080C16] text-white overflow-hidden font-sans">
      ...
      {/* 3D BACKGROUND (Watermark Hidden via Scaling) */}
      <div className="fixed inset-0 z-0 overflow-hidden">
        <div className="relative w-full h-full scale-110 origin-top">
          <iframe 
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 pb-24">
        ...
        {/* Grid displays immediately (empty if data hasn't arrived, then pops in) */}
        <motion.div 
          variants={containerVariants} 
          initial="hidden" 
          animate="show" 
          className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 pointer-events-auto"
        >
          {teamData.map((member) => (
            <motion.div 
              key={member.id} 
              ...
              className="group relative bg-[#080C16]/70 border border-white/10 rounded-2xl p-6 backdrop-blur-md overflow-hidden hover:bg-white/[0.08] hover:border-white/20 transition-all duration-300 shadow-[0_0_30px_rgba(0,0,0,0.5)] flex flex-col h-full"
            >
